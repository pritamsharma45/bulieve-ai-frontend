...
export default function PostDetail({ post }) {
  ...
  const handleComment = async (e) => {
    ...
    try {
      await createComment(post.id, user.id, comment.trim());
      setComment('');
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      ...
      <article className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
        ...
        <div className="mb-6">
          <p className="text-gray-600 dark:text-gray-300 text-lg">{post.content}</p>
          {post.media_urls && (
            <a 
        ...
        <div className="flex items-center justify-between text-gray-500 border-t pt-4">
          <button 
            onClick={handleLike}
            disabled={isLiking}
            className={`flex items-center space-x-2 ${isLiking ? 'opacity-50' : 'hover:text-red-500'}`}
          >
        ...
        {isAuthenticated && (
          <form onSubmit={handleComment} className="mt-4 pt-4 border-t">
            <div className="flex gap-2">
              ...
              >
                <Send className={`h-4 w-4 ${isCommenting ? 'animate-spin' : ''}`} />
              </button>
            </div>
          </form>
        )}
